...
LOG_DIR="$ROOT/log/er_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$LOG_DIR"
...
for data in "${datasets[@]}"; do for pred_len in "${horizons[@]}"; do
  lr=1e-3; [[ "$data" == ECL ]] && lr=3e-3; log="$LOG_DIR/er_${data}_${pred_len}_full.out"
  echo "[RUN] ER data=$data pred_len=$pred_len gpu=$GPU_ID log=$log"
  CUDA_VISIBLE_DEVICES="$GPU_ID" "$PYTHON_BIN" -u main.py --method er --root_path "$ROOT/data/" --data "$data" --features M \
    ...
    --e_layers 2 --d_ff 128 --revin 1 --individual 1 --baseline_online_lr 1e-3 --replay_buffer_size 128 --replay_batch_size 16 > "$log" 2>&1
done; done
echo "ER runs finished. log_dir=$LOG_DIR"
