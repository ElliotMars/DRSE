...
LOG_DIR="$ROOT/log/fsnet_$(date +%Y%m%d_%H%M%S)"; mkdir -p "$LOG_DIR"
...
for data in "${datasets[@]}"; do for pred_len in "${horizons[@]}"; do
  lr=1e-3; [[ "$data" == ECL ]] && lr=3e-3; log="$LOG_DIR/fsnet_${data}_${pred_len}_full.out"
  echo "[RUN] FSNet data=$data pred_len=$pred_len gpu=$GPU_ID log=$log"
  CUDA_VISIBLE_DEVICES="$GPU_ID" "$PYTHON_BIN" -u main.py --method fsnet --root_path "$ROOT/data/" --data "$data" --features M \
...
echo "FSNet runs finished. log_dir=$LOG_DIR"
